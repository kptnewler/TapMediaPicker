...
class PictureSelectionModel(private val selector: PictureSelector, chooseMode: Int) {
    private val selectionConfig: PickerConfig

    init {
        selectionConfig = PickerConfig()
        SelectorProviders.addConfigQueue(selectionConfig)
    }
    ...
    fun setFilterVideoMaxSecond(videoMaxSecond: Int): PictureSelectionModel {
        selectionConfig.filterVideoMaxSecond = (videoMaxSecond * 1000).toLong()
        return this
    }
    ...
    fun setFilterVideoMinSecond(videoMinSecond: Int): PictureSelectionModel {
        selectionConfig.filterVideoMinSecond = (videoMinSecond * 1000).toLong()
        return this
    }
    ...
    fun setSelectedData(selectedList: List<LocalMedia?>?): PictureSelectionModel {
        ...
        if (selectionConfig.selectionMode === SelectModeConfig.SINGLE && selectionConfig.isDirectReturnSingle) {
            ...
            selectionConfig.addAllSelectResult(ArrayList<Any?>(selectedList))
        }
        return this
    }
    ...
    fun forResult(call: OnResultCallbackListener<LocalMedia?>) {
        if (!DoubleUtils.isFastDoubleClick()) {
            ...
            val windowAnimationStyle: PictureWindowAnimationStyle =
                selectionConfig.selectorStyle.getWindowAnimationStyle()
    ...
    fun build(): PictureSelectorFragment {
        val activity: Activity = (selector.getActivity()
            ?: throw NullPointerException("Activity cannot be null")) as? IBridgePictureBehavior
    ...
    fun buildLaunch(
        ...
        if (activity is FragmentActivity) {
            fragmentManager = (activity as FragmentActivity).getSupportFragmentManager()
        }
